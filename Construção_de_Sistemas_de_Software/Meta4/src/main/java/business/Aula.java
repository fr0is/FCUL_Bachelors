...
public class Aula{
	...
	@Enumerated(EnumType.STRING)
	@ElementCollection
	@CollectionTable(name = "AULA_DIASSEMANA", joinColumns = @JoinColumn(name = "AULA_ID", referencedColumnName = "AULA_ID"))
	private List<DiaSemana> diasSemana;

	@ManyToOne
	@JoinColumn(name = "MODALIDADE_ID", referencedColumnName = "MODALIDADE_ID")
	private Modalidade modalidade;

	@OneToMany(cascade = ALL, fetch = EAGER)
	@JoinColumn(referencedColumnName = "AULA_ID", name = "AULA_ID")
	...
	public String infoAula(int tipoInscricao) {
		...
		LocalDateTime next24hrs = MockDateTime.currentDateTime().plusHours(24);
		if(tipoInscricao == 1) {
	 ...
	 * @throws AppException se a inscricao n tiver sido concluida por nao haver sessoes ou vagas, vai acontecer caso a inscricao seja avulsa,
	 *  as regulares tem uma vaga reservada para sessoes futuras
	 */
	public void inscreve(Inscricao inscricao) throws AppException {
		this.sortAulas();
		boolean sucesso = false;
		if(inscricao.getTipo() == 1) {
			int i = 0;
			for(AulaAtiva a: this.aulasMarcadas) {
				if(a.getDateTimeInicio().isBefore(MockDateTime.currentDateTime().plusHours(24)) && a.temVagas()) {
		...
		if(!sucesso)
			throw new AppException("A inscricao nao foi efetuada por nao existirem sessoes ou nao haver vagas");
	}
	...
	public List<AulaAtiva> gerarAulasAtivas(LocalDate data1, LocalDate data2, int nrMaxAlunos) {
		AulaAtivaFactory factory = new AulaAtivaFactory(this);
		return factory.gerarAulas(data1, data2, nrMaxAlunos);
	}
